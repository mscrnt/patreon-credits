...
When running from source (dev):
    bundle dir = project root (parent of pcg/ package)
...
def get_bundle_dir():
    """Read-only resources: fonts/, templates/, static/, assets/, .env.example"""
    if is_frozen():
        return sys._MEIPASS
    # In dev mode, project root is the parent of the pcg/ package directory
...
def get_generate_settings_path():
    return os.path.join(get_app_dir(), 'generate_settings.json')


def get_presets_dir():
    d = os.path.join(get_app_dir(), 'presets')
    os.makedirs(d, exist_ok=True)
    return d


def get_uploads_dir():
    d = os.path.join(get_app_dir(), 'uploads')
    os.makedirs(d, exist_ok=True)
    return d


def get_kofi_cache_path():
    return os.path.join(get_app_dir(), 'kofi_names.json')


def get_bmc_cache_path():
    return os.path.join(get_app_dir(), 'bmc_names.json')


def get_se_cache_path():
    return os.path.join(get_app_dir(), 'se_names.json')


def get_youtube_token_path():
    return os.path.join(get_app_dir(), 'youtube_tokens.json')
